...
pygame.display.set_caption('Bắn Tim vào Thông')  # Caption.
...
blood_strip_size = blood_strip_width, blood_strip_height = 40, 40
blood_strip_gap = 10
player_life_gap = 10
player_size = (90, 90)
elementary_enemy_size = (122, 110)
mid_enemy_size = (55, 50)
advanced_enemy_size = (60, 60)
bolt_size = (20, 32)
star_size = pill_size = shield_size = (30, 30)
player_laser_shot_size = (50, 50)
enemy_laser_shot_size = (30, 30)
...
def draw_menu():
    ...
    while True:
        ...
        draw_text(screen, '', 30, width // 2, (height // 2) + 40)
...
def draw_exit(score):
    ...
    while True:
        ...
        screen.blit(menu, (0, 0))
        draw_text(screen, 'Bạn đã bắn tim vào Thông được: ' + str(score), 30, width // 2, height // 2)
        draw_text(screen, '', 30, width // 2, height // 2 + 40)
        draw_text(screen, '', 30, width // 2, (height // 2) + 40 * 2)
...
if __name__ == '__main__':
    cap = cv2.VideoCapture('thong.mp4')
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == True:
